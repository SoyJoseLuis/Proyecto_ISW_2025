...
import {
  metaBodyValidation,
  metaQueryValidation,
} from "../validations/meta.validation.js";
import {
  handleErrorClient,
  handleErrorServer,
  handleSuccess,
...
export async function getMetas(req, res) {
  try {
    const [metas, errorMetas] = await getMetasService();

    if (errorMetas) return handleErrorClient(res, 404, errorMetas);

    metas.length === 0
      ? handleSuccess(res, 204)
      : handleSuccess(res, 200, "Metas financieras encontradas", metas);
  } catch (error) {
    handleErrorServer(res, 500, error.message);
  }
}
